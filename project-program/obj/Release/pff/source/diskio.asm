...
;------------------------------------------------------------
;	pff\source\diskio.c:14: DSTATUS disk_initialize (void)
;	-----------------------------------------
...
_disk_initialize:
	...
	ar1 = 0x01
	ar0 = 0x00
;	pff\source\diskio.c:17: if( !sd_init() )
	lcall	_sd_init
	mov	a,dpl
	mov	b,dph
	orl	a,b
	jnz	00102$
;	pff\source\diskio.c:20: Stat &= ~STA_NOINIT;
	mov	dptr,#_Stat
	movx	a,@dptr
	anl	acc,#0xfe
	movx	@dptr,a
00102$:
;	pff\source\diskio.c:24: return( Stat );
	mov	dptr,#_Stat
	movx	a,@dptr
;	pff\source\diskio.c:25: }
	mov	dpl,a
	ret
;------------------------------------------------------------
